...
源码一致性检查仅忽略 `Cargo.toml` 的 CRLF/LF 换行差异，其他内容仍严格校验。暂存目录中的 `source-before.json` 保存构建前逐文件摘要；若构建改变源码，脚本会列出变化文件并拒绝完成准备。排查后将失败目录改名保留，再重新准备，不能手动补写 `source.json` 绕过检查。
